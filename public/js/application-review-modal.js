$(() => {
  ...
  $('li').click(function() {
    ...
    $.ajax({ url: userApiUrl, type: 'GET' }).done(response => {
      ...
      $('input[value="' + response['applicationStatus'] + '"]').attr(
        'checked',
        'checked'
      );
  ...
  $('input[name="application-status-radio"]').click(function() {
    ...
    let userApiUrl =
      '/api/users/change/application-status/' +
      userEmail +
      '/' +
...
function changeApplicationStatus(
  ...
  if (oldApplicationStatus !== newApplicationStatus) {
    let oldApplicationStatusNumber = $(
      'h1.number-' + oldApplicationStatus
    ).text();
    let newApplicationStatusNumber = $(
      'h1.number-' + newApplicationStatus
    ).text();
    $('h1.number-' + oldApplicationStatus).text(
      parseInt(oldApplicationStatusNumber) - 1
    );
    $('h1.number-' + newApplicationStatus).text(
      parseInt(newApplicationStatusNumber) + 1
    );
  }
}
